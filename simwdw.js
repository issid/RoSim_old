	...
	if (m == 2 && keypressed==96) {
		...
		for (j = 1101; j <= 5121; j++) {
			...
			if ((itemeq[j][job-z] == 1 && eqp[itemdata[j][1]][l] == 1 && itemdata[j][23] != sex+1 && (itemeq[j][28] == AdvOn || itemeq[j][28] == 0)) || snequip) {
				...
				tmp2 = itemdata[j][0] + '[' + itemdata[j][3] + ']';
				...
				if (blv >= itemdata[j][6]) {
					...
					if (isforge) itemdesc[k][0] = '<font color="#EE82EE">' + itemdata[j][0] + '[' + itemdata[j][3] + ']</font>';
					else itemdesc[k][0] = '<font color="#BC8F8F">' + itemdata[j][0] + '[' + itemdata[j][3] + ']</font>';
